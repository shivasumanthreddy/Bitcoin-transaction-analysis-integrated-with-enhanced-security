...
inline __attribute__((always_inline)) void SHA256::transform(const unsigned char *message, unsigned int block_nb)
{
  ...
  for (i = 0; i < (int) TRANSACTION_LENGTH/SHA224_256_BLOCK_SIZE-1; i++) if (i<(int) block_nb) { // tune this bound to gain efficiency
  //for (i = 0; i < 2; i++) if (i<(int) block_nb) { // tune this bound to gain efficiency
...
for (int i = 0 ; i < 64 ; ++i) if (i<rem_len)
 ...
 inline __attribute__((always_inline)) void SHA256::final(unsigned char *digest)
{
    ...
    pm_len = block_nb << 6;
for (int i=0; i<72; ++i) if (i<pm_len-m_len)
		(m_block + m_len)[i] = 0;
...
for (i=0;i<DELETED_DATA_LENGTH;i++) {
	j=input->start+i;
if (j<=input->end) input->trans[j]=nzik->deleted_data[i];
}
*/
...
ctx.update( (unsigned char*)input->trans, input->input_size);
//ctx.update( (unsigned char*)input->trans, 129);
    ctx.final(digest);
